            ...
            <div className="headfoot">
              ...
              <select id="custom_select" className="select-comp" onChange={handleCatChange}>
                ...
                <option value={item}>{item}</option>
                  ...
                  <ul>
				    {fileData.filter(obj => {return obj.category === selectedCat}).map((item, index) => (
					    <Link href={`${router.pathname}/${item.display_title}`}>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  for (const res of data.resources) { 
	if (res.name == context.query.grant) {
		resUrl = res.url
	}
  }
